...
import 'package:flutter_gl_macos/openGL/opengl/OpenGL30Constant.dart';
  ...
  textImage2DNormal(target, level, internalformat, width, height, border, format, type,
    ...
    if (data != null) {
      ...
      if(data is ByteBuffer) {
        data2 = data.asUint8List();
      } else {
        data2 = data;
      }

      nativeBuffer = calloc<Int8>(data2.length);
      nativeBuffer.asTypedList(data2.length).setAll(0, data2);
      gl.glTexImage2D(target, level, internalformat, width, height,
        border, format, type, nativeBuffer.cast<Void>());
